...
import com.vita.back.api.model.data.NiceAuthDto;
import com.vita.back.api.model.data.NicePopupDto;
...
import com.vita.back.api.model.request.GetNiceEncryptTokenRequest;
...
import com.vita.back.api.model.response.GetNiceEncryptTokenResponse;
...
public class NiceCertificationServiceImpl implements NiceCertificationService {
    ...
    public NiceCirtificationResponse userJoinCertification(NiceCertificationRequest request) throws VitaException {
        ...
        if (ValidUtil.isEmpty(keyValue) || ValidUtil.isEmpty(ivValue)
            ...
            switch (request.getUserCertifyType()) {
                ...
                case "30": // 아이핀 인증의 경우
                    ...
                    break;
                default:
                    log.warn("유효하지 않은 인증구분값, {}", request.getUserCertifyType());
            ...
            if (request.getUserCertifyType().equals("10")) {
                ...
                try {
                    ...
                    mapper.updtStTo80(userCertifyDto.getUserCertifyNo());
                    log.warn("DB error (UpdtServiceInfoRegDt)", e);
        ...
        try {
            ...
            mapper.updtStTo80(userCertifyDto.getUserCertifyNo());
            log.warn("직렬화 실패", e);
            throw new VitaException(VitaCode.SYSTEM_ERROR); // 내부 에러
        }
    ...
    public RegKeyResponse selfAuthRegKey(RegKeyRequest request) throws VitaException{
        ...
        log.info("NICE Request(Header): {}", headers);
        ...
        if (resp.getRsp_cd().equals("P000")) {
            ...
            log.error("[RegKey] 암호화토큰 요청 실패: {} ", resp.getRes_msg());
            throw new VitaException(VitaCode.NETWORK_ERROR);
        }
        ...
        try {
            ...
            log.error("ERROR(substring)", e);
            throw new VitaException(VitaCode.FORMAT_ERROR); // substring 동작 에러(자릿수)
        }
        log.info("대칭키: {},", resultVal);
    ...
    public NiceCertificateAuthResponse userJoinCertificateAuth(NiceCertificateAuthRequest request) throws VitaException {
        ...
        if (request.getUserCertifyType().equals("10")) { // 핸드폰 본인인증이라면
            try {
                ...
                log.warn("DB error (GetServiceInfo)", e);
                throw new VitaException(VitaCode.DATABASE_ERROR); // DB error
            }
        } else if(request.getUserCertifyType().equals("30")) { // 아이핀 본인인증이라면
            try {
        ...
        if (ValidUtil.isEmpty(keyValue) || ValidUtil.isEmpty(ivValue)
                ...
                || ValidUtil.isEmpty(siteCodeValue) || ValidUtil.isEmpty(tokenVersionIdValue)){
            mapper.updtStTo80(request.getUserCertifyNo());
            log.info("본인인증 시간 50분 초과");
            throw new VitaException(VitaCode.CERTIFICATE_TIME_ERROR); // 본인인증 시간이 초과되었습니다.
        }
        ...
        try {
            ...
            mapper.updtStTo80(request.getUserCertifyNo());
            log.warn("인증데이터 복호화 중 에러", e);
            throw new VitaException(VitaCode.DEC_ERROR); // 내부에러
        }
        ...
        try {
            ...
            mapper.updtStTo80(request.getUserCertifyNo());
            log.warn("문자열 변환 에러", e);
            throw new VitaException(VitaCode.SYSTEM_ERROR); // 내부에러
        }
        ...
        if (request.getUserCertifyType().equals("10")) { // 휴대폰 본인인증이라면
            ...
            if (!niceAuthDto.getResultcode().equals("0000")) {
                mapper.updtStTo80(request.getUserCertifyNo());
                log.error("인증데이터 불일치");
                throw new VitaException(VitaCode.CERTIFICATE_ERROR); //본인인증이 실패했습니다.
            }
        ...
        try {
            ...
            mapper.updtStTo80(request.getUserCertifyNo());
            log.warn("DB error (UpdtHcUserCertify)", e);
            throw new VitaException(VitaCode.DATABASE_ERROR); // DB error
        }
        ...
        log.debug("niceCertificateAuthResponse: {}", niceCertificateAuthResponse);
